...
> 1は「集中化」の意味が3つあってややこしいのだが、a) 物理的な集中、は全く問題ではない。b) マイニングする「主体」の集中は不可知なので議論の対象にならない。
> 3) 固定費用の増額は「マイニングが自由市場から遠ざかる」という意味で長い目で見れば問題である可能性が高い。
> したがってBTCはコンソーシアムチェーンのような方式（PoSではない！）に移行する必要があると思うが、現時点で考えるのは早い。
> 2は、アルトチェーンが潰される可能性のほうが遥かに高いので問題ない。
> 3は、取引所などはSPVクライアント以上の信頼性する動機があるので問題ない。
