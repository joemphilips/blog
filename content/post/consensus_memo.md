...
元々Bitcoinで使われたいたProof of Work(PoW)は色々と問題があった。[^1]

1. 専用のハードウェアが有利すぎるためマイナーの集中を招く
2.  アルトチェーンが普及すると、それらのマイニングパワーを一つのチェーンに一時的に集めることで容易に51%攻撃が行えるようになる。
3. 手数料はマイニングノードが持っていくため、P2Pノードを立ち上げるモチベーションがない

> この記事を書いた当初はこれらは大きな問題だと考えていたが、現在の意見は少し違う。
> 1は「集中化」の意味が3つあってややこしいのだが、a) 物理的な集中、は大きな問題ではない。b) マイニングする「主体」の集中は不可知なので議論の対象にならない。
> 3) 固定費用の増額は「マイニングが自由市場から遠ざかる」という意味で長い目で見れば問題である可能性が高い。
> したがってBTCはコンソーシアムチェーンのような方式（PoSではない！）に移行する必要がある可能性があるが、現時点で考えるのは早い。
> 2は、アルトチェーンが潰される可能性のほうが遥かに高いので問題ない。
> 3は、取引所などはSPVクライアント以上の信頼性を確保する動機があるので問題ない。
>
> 実際のところ、PoWの「問題点」と言われているものはブロックの発行権が自由市場に有ることに伴う不可避な特徴なので、ビットコインが「マネーのプロトコル」である限り避けることは絶対にできず、
> したがって改善すべきでない。という場合が多い。
